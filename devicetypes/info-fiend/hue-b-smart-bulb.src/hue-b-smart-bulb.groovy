 ...
 *	 
 *  Version 1.5 refresh now calls doBulbsSync; cleaned update code; Added preference to select whether to use transitiontime for off() command
...
//             		log.debug "Update Needed: Current Value of colorTemperature = ${curValue} & newValue = ${val}"}	            	
	    ...
	    case "reachable":
				...
				if (curValue != val) {
	                if (val == true){
						sendEvent(name: "reachable", value: true, displayed: logTF, isStateChange: true)
					} else {
            		    sendEvent(name: "reachable", value: false, displayed: logTF, isStateChange: true)
	           		}
                }
                break
...
// GET HUE & SATURATION DATA   
	...
	sendEvent(name: "hue", value: h, displayed: false)		// , isStateChange: true)
	sendEvent(name: "saturation", value: s, displayed: false, isStateChange: true)
